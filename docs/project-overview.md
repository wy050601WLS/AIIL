...
| ~~回放式 SSE~~ | ~~ai_service.call_ai_api~~ | 已改为真流式：async 生成器逐 chunk 转发 |
...
| 方向 | 说明 | 改动范围 |
|------|------|---------|
| ~~**消息分页**~~ | ~~长会话消息分页加载，避免一次全量~~ | 已实现 |
| **图片文件存储** | base64 改为上传到服务器/对象存储 | 后端新上传端点 + 迁移 |
| ~~**对话模板**~~ | ~~预设 prompt 模板（翻译、解释、练习等）~~ | 已实现 |
...
4. ~~**真流式 SSE**~~ — 已改为 async 生成器逐 chunk 转发，首 token 延迟降至 AI 首 chunk 时间
5. ~~**消息分页**~~ — 已实现，首次加载 50 条，向上滚动自动加载更早消息
6. ~~**知识卡片编辑**~~ — 已实现，PUT /cards/{id} 支持修改内容和标签
